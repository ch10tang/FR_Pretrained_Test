...
if __name__=="__main__":
    ...
    parser.add_argument('-csv-file', type=str, default='../DataList/IJBC_FOCropped_250_250_84_ArcFace.csv', help='csv file to load image for training')
    ...
    parser.add_argument('-generate-place', type=str, default='../../04_FaceEvaluation/Experiment_IJBA_v04_ErrorAnalysis/_Features/LightCNN', help='prepared data path to run program')
                ...
                for nn in range(2):
                    ...
                    for pp in Path:
                        ...
                        if len(Fea[(Fea.Subject == int(tmp1[3])) & (Fea.Pose == Pose) & (
                                Fea.ImgNum == int(tmp1[5].split('.')[0]))].index.tolist()) > 1 or \
                                len(Fea[(Fea.Subject == int(tmp2[3])) & (Fea.Pose == Pose) &
                                        (Fea.ImgNum == int(tmp2[5].split('.')[0]))].index.tolist()) > 1:
                            exit()
