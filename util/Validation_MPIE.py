...
def Validation_MPIE(Model, epoch, device, save_dir, cfg, Error_Flag=False):
    ...
    # Load augmented data
    transform = transforms.Compose([transforms.Resize([INPUT_SIZE[0], INPUT_SIZE[0]]),
                                    transforms.ToTensor(),
                                    transforms.Normalize((0.5, 0.5, 0.5),
                                                         (0.5, 0.5, 0.5))])
    transformed_dataset = FaceIdPoseDataset(csv_file, data_place, transform=transform)
    dataloader = DataLoader(transformed_dataset, batch_size=5, shuffle=False)  # , num_workers=6)
